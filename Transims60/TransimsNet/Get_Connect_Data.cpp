...
bool TransimsNet::Get_Connect_Data (Connect_File &file, Connect_Data &connect_rec)
{
	...
	if ((delete_link_flag && delete_link_range.In_Range (file.To_Link ())) ||
		(update_link_flag && update_link_range.In_Range (file.To_Link ()))) {
		xconnect++;
		return (false);
	}
	...
	return (Data_Service::Get_Connect_Data (file, connect_rec));
}

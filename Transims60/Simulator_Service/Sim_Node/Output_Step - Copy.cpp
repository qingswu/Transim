...
if (step.sim_travel_ptr->First_Cell () == 5719) exe->Write (1, " output_step link=") << to_index << " lane=" << to_lane << " driver=" << step.Traveler ();
	...
	if (step.Status () == 1 || step.Problem () == DEPARTURE_PROBLEM || step.Problem () == ARRIVAL_PROBLEM) {
		if (step.sim_travel_ptr->Person () == 0) {
			...
			if (step.sim_travel_ptr->First_Cell () >= 0) {
				...
				if (sim->event_output.In_Range (VEH_LOST_EVENT, sim_plan_ptr->Mode (), subarea)) {
					...
					if (step.veh_cell_ptr == 0) {
						if (step.sim_travel_ptr->First_Cell () >= 0) {
							step.veh_cell_ptr = &sim->veh_cell_array [step.sim_travel_ptr->First_Cell ()];
							step.veh_cell_ptr->Location (&to_index, &to_lane, &to_offset);
						}
					} else {
						step.veh_cell_ptr->Location (&to_index, &to_lane, &to_offset);
					}
	...
	if (step.Problem () > 0) {
		...
		if (!sim->select_problems || sim->problem_range.In_Range (step.Problem ())) {
			if (sim->param.problem_flag && sim_plan_ptr != 0) {
				if ((int) step.size () > 0) {
					cell_itr = --step.end ();
					to_index = cell_itr->link;
					to_lane = cell_itr->lane;
					to_offset = cell_itr->offset;
				} else if (step.veh_cell_ptr == 0) {
					if (step.sim_travel_ptr->First_Cell () >= 0) {
						step.veh_cell_ptr = &sim->veh_cell_array [step.sim_travel_ptr->First_Cell ()];
						step.veh_cell_ptr->Location (&to_index, &to_lane, &to_offset);
					}
	...
	if (step.veh_cell_ptr != 0) {
		...
		if (step.veh_cell_ptr->Lane () >= 0) {
			sim->Check_Output (step);
		}

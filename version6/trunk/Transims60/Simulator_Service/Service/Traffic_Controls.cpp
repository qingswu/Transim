...
void Simulator_Service::Traffic_Controls (bool initialize)
{
	...
	if (initialize) {
		...
		for (index=0, sig_itr = signal_array.begin (); sig_itr != signal_array.end (); sig_itr++, index++) {
			int_itr = sig_itr->nodes.begin ();
			node_ptr = &node_array [*int_itr];

			sim_signal_ptr = &sim_signal_array [index];
		}
	}
	...
	for (index=0, signal_itr = sim_signal_array.begin (); signal_itr != sim_signal_array.end (); signal_itr++, index++) {
		if (time_step < signal_itr->End_Time () && signal_itr->End_Time () < MAX_INTEGER) {
		...
		for (time_itr = signal_ptr->begin (); time_itr != signal_ptr->end (); time_itr++) {
			if (time_itr->Start () > time_step || time_step >= time_itr->End ()) continue;
			...
			for (timing = 0, timing_itr = signal_ptr->timing_plan.begin (); timing_itr != signal_ptr->timing_plan.end (); timing_itr++, timing++) {
				...
				offset = (time_step - (time_itr->Start () + offset)) % cycle;
				if (offset < 0) offset += cycle;

				time = time_step - offset;
				barrier = offset = 0;
				bar = MIN (1, max_bar);

				for (; bar <= max_bar; bar++) {
					end_time = time + max_bar_time [bar];
					if (time_step >= time && time_step < end_time) {
						offset = time_step - time;
						barrier = time;
						break;
					}
					time = end_time;

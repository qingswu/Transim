...
bool Sim_Node_Process::Check_Behind (Sim_Veh_Data veh_data, Travel_Step &step, int num_cells, bool use_flag)
{
	...
	if (dir_index == step.Dir_Index ()) {
		if (step.sim_dir_ptr == 0) {
			step.sim_dir_ptr = &sim->sim_dir_array [dir_index];
		}
	...
	min_gap = num_cells - 1;
	max_gap = num_cells + MAX ((step.veh_type_ptr->Length () / sim->param.cell_size), 1);
	...
	if ((speed == 0 && gap == 0) || min_step == max_step) {
		...
		if (sim->param.traveler_flag) permission *= sim->param.traveler_fac.Best (sim_travel_ptr->sim_plan_ptr->Type ());
	...
	if (link_flag) {
		...
		if (leg_itr == sim_plan_ptr->end () || leg_itr->Connect () < 0) return (true);

		sim_con_ptr = &sim->sim_connection [leg_itr->Connect ()];

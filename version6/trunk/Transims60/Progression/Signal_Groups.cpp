		...
		while (link_dir > 0) {
			...
			if (in_dir_ptr != NULL) {
				...
				if (signal_ptr == NULL || signal_ptr->Node () != node) {
					...
					if (offset_ptr == NULL) {
						Error (String ("Signal at Node %d was Not Found") % node);
					}
					...
					if (!group_ptr->Add_Link (&link_rec)) {
						Error (String ("Adding Link to Group %d") % group);
					}
					ttime = length = hours = 0;
				}
...
link_error:
	Error (String ("Link %d on Link Equivalence Group %d was Not Found") % link % group);
}

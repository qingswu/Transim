...
int Progression::Set_Offset (int group, bool report_flag, bool eval_flag)
{
	...
	while (group) {
		...
		if (report_flag) {
			Print (2, String ("**** Period %d, Group %d ") % period % group_ptr->Group ());

			if (Link_Equiv_Flag ()) {
				Print (0, String ("[%s] ") % link_equiv.Group_Label (group_ptr->Group ()));
			}
			if (weight != 1.0) {
				Print (0, String ("Weight %.2lf ") % weight);
			}
			Print (0, "****");
		}
		...
		for (link_ptr = group_ptr->First_Link (); link_ptr; link_ptr = group_ptr->Next_Link ()) {
			...
			if (report_flag) Print (1, String ("Node=%8d, Arrival=%3d") % link_ptr->Node () % offset);
			...
			if (signal_ptr->Offset () < 0) {
				...
				if (thru_green > 0) {
					...
					if (report_flag) Print (0, String (", %3d%%") % (int) (max_green * 100 + 0.5) % FINISH);
				} else {
			...
			if (report_flag) Print (0, String (", Departure=%3d") % signal_ptr->Offset ());
			...
			for (next = signal_ptr->First_Group (); next != 0; next = signal_ptr->Next_Group ()) {
				if (next != group) {
                    ...
                    if (report_flag) {
						if (first) {
							Print (0, String (", Intersect= %d") % next_ptr->Group ());
							first = false;
						} else {
							Print (0, String (", %d") % next_ptr->Group ());
						}
					}
				}
			}
	...
	if (report_flag) {
		...
		Print (2, String ("Period %d Performance Measure = %d") % period % abs (performance));
	}
	return (performance);
}

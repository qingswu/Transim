...
void Trip_Gap_Data::Initialize (void)
{
	...
	gap_sum_array.assign ((num_periods + 1), gap_sum);
}
...
void Trip_Gap_Data::Add_Trip_Gap_Data (Dtime tod, double input, double compare)
{
	...
	double diff_sq = abs_diff * abs_diff;

	Gap_Sum *gap_ptr;

	gap_ptr = &gap_sum_array [num_periods]; 

	gap_ptr->count++;
	gap_ptr->current += input;
	gap_ptr->previous += compare;
	gap_ptr->abs_diff += abs_diff;
	gap_ptr->diff_sq += diff_sq;
	if (abs_diff > gap_ptr->max_gap) gap_ptr->max_gap = abs_diff;

	//---- period summary ----

	if (period_flag) {
		gap_ptr = &gap_sum_array [period];
...
void Trip_Gap_Data::Trip_Gap_Report (int number)
{
	...
	for (period = 0; period <= num_periods; period++) {
		...
		exe->Print (1, String ("%-12.12s %9d %14.0lf %14.0lf %14.0lf %12.6lf") %
			buffer % gap_ptr->count % (gap_ptr->current / factor) % (gap_ptr->previous / factor) %
			(gap_ptr->abs_diff / factor) % ((gap_ptr->previous > 0) ? (gap_ptr->abs_diff / gap_ptr->previous) : 0.0));
	}
	exe->Header_Number (0);
}
...
void Trip_Gap_Data::Write_Trip_Gap_File (void)
{
	...
	for (period=0; period <= num_periods; period++) {
		...
		file << (String ("%s\t%d\t%.2lf\t%.2lf\t%.2lf\t%.6lf\n") % buffer % gap_ptr->count % 
			(gap_ptr->current / factor) % (gap_ptr->previous / factor) % (gap_ptr->abs_diff / factor) % 
			((gap_ptr->previous > 0) ? (gap_ptr->abs_diff / gap_ptr->previous) : 0.0));
	}
	exe->End_Progress ();
	gap_file.Close ();
...
void Trip_Gap_Data::Merge_Data (Trip_Gap_Data &data)
{
	...
	for (i=0, gap_itr = gap_sum_array.begin (); gap_itr != gap_sum_array.end (); gap_itr++, i++) {
		gap_ptr = &data.gap_sum_array [i];

		gap_itr->count += gap_ptr->count;
		gap_itr->current += gap_ptr->current;
		gap_itr->previous += gap_ptr->previous;
		gap_itr->abs_diff += gap_ptr->abs_diff;
		gap_itr->diff_sq += gap_ptr->diff_sq;
		if (gap_ptr->max_gap > gap_itr->max_gap) gap_itr->max_gap = gap_ptr->max_gap;
	}
}

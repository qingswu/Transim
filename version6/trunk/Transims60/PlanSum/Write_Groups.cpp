//*********************************************************
//	Write_Groups.cpp - write access group file
//*********************************************************
...
void PlanSum::Write_Access_Groups (void)
{
	...
	Show_Message ("Writing Access Groups -- Record");
	Set_Progress ();
	...
	fstream &file = access_group_file.File ();

	for (map_itr = access_group.begin (); ; map_itr++) {
		Show_Progress ();

		if (map_itr != access_group.end ()) {
			xfer_io = map_itr->first;
			trips = map_itr->second;
		}
		if (map_itr == access_group.end () || last_group != xfer_io.group) {
			if (last_group > 0) {
					...
					if (Notes_Name_Flag ()) {
						...
						for (d_itr=od_map.begin (); d_itr != od_map.end (); d_itr++) {
							...
							if (d_itr->first.second == 0) {
								...
								label += line_equiv.Group_Label (d_itr->first.second);
					...
					for (o_itr = od_map.begin (); o_itr != od_map.end (); o_itr++) {
						...
						if (Notes_Name_Flag ()) {
							...
							if (o_itr->first.second == 0) {
								...
								label += line_equiv.Group_Label (o_itr->first.second);
			...
			if (map_itr == access_group.end ()) break;
			...
			for (group_itr = map_itr; group_itr != access_group.end (); group_itr++) {
				od = group_itr->first;
